...
cv.createTrackbar("CannyL", "HOUGH", 211, 1000, null)
cv.createTrackbar("CannyH", "HOUGH", 280, 1000, null)
...
while True:
    ...
    canny_img = cv.Canny(gray_image, canny_l, canny_h, apertureSize=3, L2gradient=True)


    linesP = cv.HoughLinesP(canny_img, rho=2, theta=np.pi/180, threshold=hough_T, minLineLength=hough_Min, maxLineGap=hough_Gap)
